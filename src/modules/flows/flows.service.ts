...
export class FlowsService {
  ...
  async findAll(): Promise<FlowResponseDto[]> {
    const flows = await this.flowRepository.find({
      relations: ['role', 'category', 'role.area'],
    });
    ...
    return flows.map((flow) =>
      plainToInstance(FlowResponseDto, flow, {
  ...
  async findById(id: number): Promise<FlowResponseDto> {
    ...
    return plainToInstance(FlowResponseDto, flow, {
      excludeExtraneousValues: true,
    });
  }

  async createFlow(createFlow: CreateFlowDto): Promise<FlowResponseDto> {
    const existingRole = await this.roleService.findById(
      createFlow.id_role,
    );
    ...
    const duplicateRole = await this.flowRepository.findOne({
      where: {
        id_category: createFlow.id_category,
        id_role: createFlow.id_role,
      },
    });
  ...
  async updateFlow(
    ...
    // 🔹 VALIDACIÓN DE ROL
    if (
      updateFlow.id_role !== undefined &&
      updateFlow.id_role !== existingFlow.id_role
    ) {
      const existingRole = await this.roleService.findById(
        updateFlow.id_role,
      );
      ...
      const duplicateRole = await this.flowRepository.findOne({
        where: {
          id_category: existingFlow.id_category,
          id_role: updateFlow.id_role,
        },
      });
    ...
    if (
      updateFlow.sequence !== undefined &&
      updateFlow.sequence !== existingFlow.sequence
    ) {
      const duplicateSequence = await this.flowRepository.findOne({
        where: {
          id_category: existingFlow.id_category,
          sequence: updateFlow.sequence,
        },
      });

      if (duplicateSequence) {
        throw new BadRequestException(
          `Ya existe un flujo con la secuencia ${updateFlow.sequence} en la categoría ${existingFlow.id_category}`,
        );
      }
    ...
    return plainToInstance(FlowResponseDto, updatedFlow, {
      excludeExtraneousValues: true,
    });
  }
  ...
  async deleteFlow(): Promise<void> {
    throw new BadRequestException(
      'Los flujos no pueden eliminarse una vez creados',
    );
  }

  // 🔥 MÉTODO CORREGIDO (EL QUE TE FALLABA)
  async findByCategoryOrderBySequence(
    idCategory: number,
  ): Promise<Flow[]> {
    const flows = await this.flowRepository.find({
    ...
    if (!flows || flows.length === 0) {
      throw new NotFoundException(
        `No se encontraron flujos para la categoría ${idCategory}`,
      );
    }
