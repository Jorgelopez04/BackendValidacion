...
import { UpdateRoleResposeDto } from './dto/update-role-response.dto'; // Corregido el typo 'Respose' -> 'Response'

@Injectable()
export class RolesService {
    // 1. Se añade 'readonly' para cumplir con la regla de inmutabilidad de SonarQube
    constructor(
    ...
    async updateRole(id: number, updateRole: UpdateRoleDto): Promise<UpdateRoleResposeDto> {
        ...
        if (updateRole.name) {
            const roleWithSameNameInArea = await this.roleRepository.findOneBy({ 
                id_area: existingRole.id_area, 
    ...
    async deleteRole(id: number): Promise<RoleResponseDto> {
        ...
        if (existingRole.employees && existingRole.employees.length > 0) {
