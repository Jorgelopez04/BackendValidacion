describe('Backend service task state cascade', () => {
  it('creates a task, starts it and validates cascade through product and order', () => {
      ...
      cy.request({
        ...
        cy.request({
          ...
          if (createResponse.status !== 201) {
            cy.log('Skipped cascade flow because task could not be created');
            return;
          }
          ...
          cy.screenshot('tasks-cascade-flow');
        });
      });
    });
  });
