describe('CompleteTask() Regression Tests', () => {
  it('should complete a task via PATCH /tasks/:id/complete', () => {
        ...
        cy.request({
          ...
          cy.request({
            ...
            cy.request({
              ...
              cy.screenshot('completetask-regression');
            });
          });
        });
      });
  ...
  it('should validate end_date is set on task complete', () => {
    cy.env(['adminCc', 'adminPassword']).then(({ adminCc, adminPassword }) => {
      cy.request({
        method: 'POST',
        url: '/auth/login',
          ...
          cy.request({
            ...
            cy.request({
              ...
              if (response.status === 201) {
                ...
                expect(response.body.data.end_date).to.not.be.null;
              }

              cy.screenshot('completetask-end-date-validation');
            });
          });
        });
      });
